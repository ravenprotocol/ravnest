...
model = torch.jit.load('submod_0.pt')
...
if __name__ == '__main__':
    node = Node(name='n0', local_host=host, local_port=port, model=model, optimizer=torch.optim.Adam, forward_target_host='0.0.0.0', forward_target_port=8081)
    node.start()
    batch_size = 64
    epochs = 50
    tensor_id = 0
    ...
    pred = node.no_grad_forward_compute(tensors=torch.tensor(X_test, dtype=torch.float32))
